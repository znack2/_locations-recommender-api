  getRecommendation: async (req, res, next) => {
    try {
      ...
      const getLocationData2 = async locationsData => {
      ...
      async function setLocationData2(locationsData) {
          let result = [];

          while (locationsData.length) {
              let batch = locationsData.splice(0,5); // make 3 requests in parallel
      ...
      const locationsData2 = await setLocationData2(locationsData);


      const middle = locationsData2.filter(Boolean);
